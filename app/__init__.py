...
from app import models, soap_views
...
if not app.debug:
    ...
    mail_handler = SMTPHandler((LOCAL_SMTP['SERVER'], LOCAL_SMTP['PORT']), 'no-reply@' + LOCAL_SMTP['SERVER'],
                               ADMINS, 'mc-utils crash', credentials)
...
if not app.debug:
    ...
    file_handler = RotatingFileHandler('tmp/mc-utils.log', 'a', 1 * 1024 * 1024, 10)
    file_handler.setFormatter(logging.Formatter('%(asctime)s %(levelname)s: %(message)s [in %(pathname)s:%(lineno)d]'))
    app.logger.setLevel(logging.INFO)
    file_handler.setLevel(logging.INFO)
    app.logger.addHandler(file_handler)
    app.logger.info('mc-utils startup')
